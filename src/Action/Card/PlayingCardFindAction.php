...
class PlayingCardFindAction
{
    protected CardFinderService $service;

    protected JsonRenderer $renderer;

    /**
     * @param CardFinderService $service
     * @param JsonRenderer $jsonRenderer
     */
    public function __construct(CardFinderService $service, JsonRenderer $jsonRenderer)
    {
    ...
    ): ResponseInterface
    {
        ...
        if (array_key_exists("date", $args)) {
            $date = (int)$args["date"];
            $data = $this->service->findPlayingCardByDate($date);
        } else {
            $data = $this->service->findPlayingCard();
        }
